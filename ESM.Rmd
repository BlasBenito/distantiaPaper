...
#write table
kable(eemian.sites.table, digits = 2, booktabs = T, col.names = c("Site", "Lat. (º)", "Lon. (º)", "Elev. (m)", "Pollen types", "Samples", "Reference*"), caption = "Pollen sequences publicly available representing the Last Interglacial in Central Europe.", row.names = FALSE) %>% kable_styling(latex_options = c("HOLD_position", "striped"))
```
...
```{r, message = FALSE, warning = FALSE, fig.width=4, fig.height=3, fig.cap = "Similarity between pollen sites.  Colors represent latitude, and edge thickness represent similarity, computed as 1/dissimilarity to highlight stronger similarities.", echo = FALSE}
...
kable(aic, digits = 2, booktabs = T, caption = "AIC values of the complete model (first row) and each one of the univariate models.") %>% kable_styling(latex_options = c("HOLD_position", "striped"))
```

According to the AIC results, the model based on the elevation difference is the one better explaining ψ. The plot below shows the model fit and the data.
...
```{r, echo = FALSE}
#printing table
kable(psi.drop, col.names = "Psi drop (%)", booktabs = T, caption = paste("Drop in dissimilarity (expressed as a percentage of the dissimilarity when computed with all variables) when omitting each variable of the sequences", A, "and", B, sep = " ")) %>% kable_styling(latex_options = c("HOLD_position", "striped"))
```

According to the table, the following pollen types are the ones contributing the most to the dissimilarity between `r paste(A)` and `r paste(B)`: `r names(psi.drop[,1][which(psi.drop[,1] > 1)])`.
...
```{r, fig.width=7, fig.height=3, fig.cap = "Dissimilarity change when removing one predictor at a time. ψ values of the complete datasets for each pair of sites are indicated by dashed lines. An increment in ψ when a given variable is removed indicates that the variable contributes to the similarity between sites."}
...
These operations are performed by the functions available in the file *sensitivity_functions.R* The function *repetitionsPsi()* computes ψ on a given dataset and its modified copy a number of repetitions, while *sensitivityPsi()* runs each whole experiment.
...
The sensitivity analysis below computes ψ 30 times (repetitions) for each combination of parameters contained in the parameters dataframes. On each repetition, different data-points are modified for **Scenario 1**, and different rows are removed for **Scenario 2**. ψ values obtained from the sensitivity analysis are aggregated through repetitions, and the mean and standard deviation across repetitions are available in the output dataframe of the sensitivity analysis.
...
We plot changes in ψ mean and standard deviation across number of data-points modified for each method and combination of transformation and distance metric.
...
```{r, fig.width=7, fig.height=4, fig.cap = "ψ mean and standard deviation across number of data-points modified for the pollen dataset.", echo = FALSE}
plot.pollen <- ggplot(
...
```{r, fig.width=7, fig.height=7, fig.cap = "ψ mean and standard deviation across number of removed samples for the climate dataset.", echo = FALSE}
...
```{r, fig.width=7, fig.height=7, fig.cap = "ψ mean and standard deviation across number of removed samples for the pollen dataset.", echo = FALSE}
