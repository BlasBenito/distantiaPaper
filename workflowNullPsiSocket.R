...
#'
#' @export
workflowNullPsiSocket <- function(sequences = NULL,
                        grouping.column = NULL,
                        time.column = NULL,
                        exclude.columns = NULL,
                        method = "manhattan",
  ...
  psi.real <- workflowPsi(
   ...
   ######################################
    ...
    if(parallel.execution == TRUE){
      ...
      my.cluster <- snow::makeCluster(n.cores)
      doSNOW::registerDoSNOW(my.cluster)
      progress.bar <- utils::txtProgressBar(max = repetitions, style = 3)
      progress <- function(n) utils::setTxtProgressBar(progress.bar, n)
      ...
      `%dopar%` <- foreach::`%do%`
      on.exit(`%dopar%` <- foreach::`%dopar%`, add = TRUE)
    }

    #parallelized loop
    #---------------------------------------
    psi.random <- foreach::foreach(i = 1:repetitions, .options.snow = opts) %dopar% {

      #printing log
      Log("Processing iteration %d of %d", i, repetitions)
